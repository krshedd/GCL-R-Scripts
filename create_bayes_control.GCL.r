create_bayes_control.GCL <- function(sillyvec, loci, mixvec, baseline_name, nreps = 40000, nchains, groupvec, priorvec, initmat, dir, seeds = matrix(sample(seq(10000), 3*nchains), nrow = 3), thin = c(1, 1, 1), mixfortran, basefortran, switches = "F T F T F T F"){
  ...
  #   mixfortran - the fortran format of the mixture files; this is returned by create_bayes_mixture.GCL
  #   basefortran - the fortran format of the baseline file; this is returned by create_bayes_baseline.GCL
  ...
  # CombineLoci.GCL(sillyvec = sillyvec, markerset = c("Ots_MHC1", "Ots_MHC2"))
  # base_fortran <- create_bayes_baseline.GCL(sillyvec = sillyvec, loci = loci, dir = "bayes/baseline", baseline_name = "SusitnaChinook31pops82loci", ncores = 8)
  # mix_fortran <- create_bayes_mixture.GCL(mixvec = c("Susitna2018", "Susitna2019"), loci = loci, dir = "bayes/mixture", ncores = 8)
  # inits <- RandomInits.GCL(groupvec = groupvec3, groupweights = rep(1/max(groupvec3), max(groupvec3)), nchains = 5)
  # priorvec <- Prior.GCL(groupvec = groupvec3, groupweights = rep(1/max(groupvec3), max(groupvec3)))
