...
if(FALSE){##
  ...
  sapply(DuplicateCheckReportSummary[nDupsBySilly >=1], function(silly) { 
    silly <- silly %>% mutate(ID1 = factor(ID1), ID2 = factor(ID2)) #Andy added this line 11/16/22 
    if(1 %in% abs(as.numeric(levels(silly$ID1)) - as.numeric(levels(silly$ID2)))) {"Sequential IDs found as duplicates, check 'DuplicateCheckReportSummary' for duplicated rows"} else {"Duplicates exist, but IDs do not appear sequential"}})
