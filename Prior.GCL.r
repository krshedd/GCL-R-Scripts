Prior.GCL <- function(groupvec, groupweights, minval = 0.01, type = c("BAYES", "rubias")[1], sillyvec = NULL){
  ...
  #
  # Note: The BAYES prior object is supplied to the prior argument in create_bayes_control.GCL()
  #       The rubias prior object is supplied to the pi_prior argument in run_rubias_mixture()
  #
  # Examples~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  #
  # BAYES_prior <- Prior.GCL(groupvec = groupvec, groupweights = rep(1/max(groupvec), max(groupvec)), minval = 0.01, type = "BAYES", sillyvec = NULL)
  #
  # rubias_prior <- Prior.GCL(groupvec = groupvec, groupweights = rep(1/max(groupvec), max(groupvec)), minval = 0.01, type = "rubias", sillyvec = sillyvec)
  # 
  # Note~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  # 
