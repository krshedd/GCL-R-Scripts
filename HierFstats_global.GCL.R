HierFstats_global.GCL <- function(levels, genotypes){
  ...
  # 
  #   temporal_collections <- temporal_collections.GCL(sillyvec = sillyvec104, region = region, min.samps = 50, sep = ".")
  #
  #   CombineLoci.GCL(sillyvec = sillyvec187, markerset =  c("Oki_102213-604", "Oki_101119-1006"))# Combining two loci to test haploid calcs
  # 
  #   fstat.dat <- create_hierfstat_data.GCL(sillyvec = temporal_collections$silly, region = temporal_collections$region, pop = temporal_collections$pop, loci = c("Oki_102213-604.Oki_101119-1006", loci81), ncores = 8)
  #
  #   HierFstats_global.GCL(levels = fstat.dat[,1:3], genotypes = fstat.dat[,-c(1:3)])
  #
  #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  if(!require("pacman")) install.packages("pacman"); library(pacman); pacman::p_load(tidyverse, hierfstat) #Install packages, if not in library and then load them.
