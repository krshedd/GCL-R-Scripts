FreqFisPlot4SNPs.GCL <- function(sillyvec, loci, groupvec, alpha = 0.05, groupcol = NULL, file = NULL, group.pch = 19, point.size = 1, pval.cex = 3, pval.digits = 2, line.width = .5, ncores = 4){
  ...
  #   
  #  FreqFisPlot4SNPs.GCL(sillyvec = sillyvec227, loci = loci91, groupvec = groupvec19, alpha = 0.05, groupcol = grcol, file = "./FreqFisPlot.pdf", ncores = 8)
  #    
  #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  if(!require("pacman")) install.packages("pacman"); library(pacman); pacman::p_load(tidyverse, HardyWeinberg, parallel, foreach, doParallel, grid)  # Install packages, if not in library and then load them
  ...
  Freq <- suppressMessages(FreqPop.GCL(sillyvec = sillyvec, loci = loci, ncores = ncores))
