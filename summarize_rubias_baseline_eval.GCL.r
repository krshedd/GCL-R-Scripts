summarize_rubias_baseline_eval.GCL <- function(mixvec, sample_sizes, method = c("MCMC", "PB", "both")[1], group_names = NULL, group_names_new = NULL,
                                               groupvec = NULL, groupvec_new = NULL, path = "rubias/output", alpha = 0.1, 
                                               burn_in = 5000,  threshold = 5e-7, ncores = 4){
  #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  #
  #   This function is a wrapper for custom_combine_rubias_output(). The function creates a tibble of sample sizes for creating baseline and mixture files for baseline evaluation tests.
  #   
  #   The output from this function can be used by CreateRubiasBaselineEval.GCL() to write out mixture and baseline files
  ...
  # 
  #   sample_sizes - a tibble produced by BaselineEvalSampleSizes.GCL() containing the following variables: test_group, scenario, repunit, and samps
  #
  ...
  #
  #   see custom_combine_rubias_output() documentation for the remaining inputs.
  #
  # Outputs~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  #
  ...
  if(method == "MCMC"){
    ...
    estimates <- custom_combine_rubias_output(rubias_output = NULL, mixvec = mixvec, group_names = group_names, group_names_new = group_names_new,
