ProjectGenotypeReportStats.GCL <- function(report = NULL, in.file = NULL, out.file = "GenotypeReportQC.xlsx"){  
  ...
  # Inputs~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  #   
  #  report - a project genotypes report object created by GenotypeReport.GCL (i.e., genotypes pulled using project name)
  #   
  #  in.file - the file path to an unmodified genotypes report .csv file created by GenotypeReport.GCL
  #
  ...
  # 
  #  Sheet1 - the report produced by GenotypeReport.GCL when pulling by project_name with "Count of 0/0" and "Success Rate" columns added and conditional formatting.
  #  
  ...
  # project_name <- "P061"
  #
  # GenotypeReport <- GenotypeReport.GCL(sillyvec = NULL, loci = NULL, path = "GenotypeReport_test.csv", username = username, password = .password, project_name = project_name, open.file = FALSE)
  #
  # ProjectGenotypeReportStats.GCL(report = GenotypeReport, in.file = NULL, out.file = "FormattedGeneotypesReport_test.xlsx")
  #
  # ProjectGenotypeReportStats.GCL(report = NULL, in.file = "GenotypeReport.csv", out.file = "FormattedGeneotypesReport_test.xlsx")
  #
  #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  if(!require("pacman")) install.packages("pacman"); library(pacman); pacman::p_load(tidyverse, openxlsx)  # Install packages, if not in library and then load them.
