Test_LD.GCL <- function(path, genepopFiles, dememorizations = 10000, batches = 100, iterations = 5000, ncores = parallel::detectCores() - 1){
  ...
  #
  # Test_LD.GCL (genepopFiles = my.files, path = "GENEPOP", batches = 1, iterations = 1, ncores = 18)
  #
  #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  if(!require("pacman")) install.packages("pacman"); library(pacman); pacman::p_load(fs, tidyverse, parallel, doParallel, foreach, genepop) #Install packages, if not in library and then load them.
  ...
  LDout <- foreach::foreach(f = genepopFiles, .packages ="magrittr", .export = c("ReadGenepopDis.GCL")) %dopar% { 
    ...
    ReadGenepopDis.GCL(file = paste0(f, ".DIS"), loci = loci, sillyvec = sillyvec)
