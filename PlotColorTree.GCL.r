PlotColorTree.GCL <- function(color.tree, rm.neg.branch = TRUE, type = "phylogram", scale = TRUE, show.tip.label = TRUE, edge.width = 3, adj = NULL, cex = 1, font = 1, label.offset = 0, gen.dist = "FST", ...){
  #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  #
  ...
  # Inputs~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  #   
  #   color.tree - a color tree object created with treeColor.GCL
  #
  ...
  #   
  #   Fst36 <- ReadGenepopPWFST.GCL(file = "GENEPOP/Sustina36pops84loci.MIG", popnames = Pooling_1_pops$location)
  #   
  #   FstTree <- ape::nj(Fst36)
  #   
  #   groupvec <- factor(Pooling_1_pops$group,unique(Pooling_1_pops$group)) %>% as.numeric()
  #
  #   colortree <- treeColor.GCL(tree = FstTree, currentnames = FstTree$tip.label, treenames = FstTree$tip.label, groupvec = groupvec, regioncol = grcol)
  # 
  #   PlotColorTree.GCL(color.tree = colortree, 
  #                     rm.neg.branch = TRUE, 
  #                     type = "phylogram", 
  #                     scale = TRUE,
  #                     show.tip.label = TRUE, 
