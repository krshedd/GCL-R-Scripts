BAYES_IndividualAssignmentSummary.GCL <- function(group_names, groupvec, mixnames, maindir, nchains, nreps, burn = 0.5, thin = 100){
  ...
  #
  # mixnames - a character vector indicating mixtures names (i.e. name of folder where
  ...
  #     mixname - the name of the mixture;
  #     id  - the individual id;
  #     and a variable for each group in group_names containing the proportion of thinned MCMC iterations
  #     in which an individual was assigned to a specific reporting group.
  # Note~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  #     Individual ids come from FK_FISH_ID if the mixture .gcl object exists,
  #     otherwise they are sequential. Mixture objects can be either new- (tibble) or old-style (list) .gcl objects.
  #
  #     BAYES will drop mixture individuals from the analysis if they contain an allele that does not exist in the baseline. 
  #     Check the BAYES summary files for each mixture and remove dropped individuals from the mixture .gcl objects before  
  #     running this function. Otherwise, the function will produce an error message stating that the number of individuals
  #     in the mixture object exceeds then number of individuals in the BAYES output.
  #
  # Example~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
