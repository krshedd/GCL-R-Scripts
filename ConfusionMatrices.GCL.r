ConfusionMatrices.GCL <- function(reference, gen_start_col = 5, output = c("group_group", "pop_group", "pop_pop")[1]){
  ...
  ####################################################################################################################################################################################################################################################################
  #
  # This function is a wrapper for rubias::self_assign()
  # It takes the genetic likelihood profile produced by rubias::self_assign()
  # and summarizes into tibbles at the group by group, group by pop, and pop by pop level.
  ...
  # Input parameters~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  # 
  # reference - a rubias baseline object created by create_rubias_baseline()
  #
  # gen_start_col - the first column containing genetic data in the baseline object. 
  #                 This will usually be the default, unless the baseline object is 
  #                 modified with additional variables.
  # 
  # output - a character vector of summary tibbles you want to produce. Must be one or 
  #          more of the following: "pop_pop", "pop_group", "group_group"
  #          The default is "group_group".
  #
  # Output~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  # Named List of tibble(s) in long format:
  #
  #  group_group - a tibble containing the following variables:
  #         1) repunit - the known repunit(aka reporting group)
  #         2) inferred_repunit - the inferred repunit 
  #         3) mean_group_group_scaled_like - the average probability of each individual from a group originating from a pop in that group
  #
  #  pop_group - a tibble containing the following variables:
  #         1) collection - the known population
  #         2) inferred_repunit - the inferred repunit 
  #         3) mean_pop_group_scaled_like - the average probability of each individual from a pop originating from a pop in that group
  #         
  #  pop_pop - a tibble containing the following variables:
  #         1) pop - the known population
  #         2) inferred_pop - the inferred repunit 
  #         3) mean_pop_pop_scaled_like - the average probability of each individual from a pop originating from a pop
  #
  #  **Note: to visualize the output from this function in a heatmap (ggplot2::geom_tile) use plot_confusion_matrix.GCL()**
  # 
  # Example~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  #
