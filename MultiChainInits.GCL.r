MultiChainInits.GCL <- function(npops, nchains, prop = 0.9, type = c("BAYES", "rubias")[1], sillyvec = NULL){
  ...
  #
  # Note: The BAYES initial start value object is supplied to the initmat argument in create_bayes_control.GCL()
  #       The rubias initial start value object is supplied to the pi_init argument in run_rubias_mixture(); however,[ 
  ...
  # 
  # BAYES_inits <- MultiChainInits.GCL(npops = length(sillyvec), nchains = 5, prop = 0.9, type = "BAYES", sillyvec = NULL)
  #
  # rubias_inits <- MultiChainInits.GCL(npops = length(sillyvec), nchains = 5, prop = 0.9, type = "rubias", sillyvec = sillyvec)
  #
  #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  if(type == "rubias"){
    ...
    initmat <- lapply(1:nchains, function(chain){
     ...
     tibble::tibble(collection = sillyvec, pi_init = Prior.GCL(groupvec = groupvec, groupweights = GroupWeights[, chain], minval = 0) %>% as.numeric())
